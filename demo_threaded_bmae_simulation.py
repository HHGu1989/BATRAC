#!/usr/bin/env python3
"""Run a threaded BMAE registration + batch authentication simulation."""
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Batch size per ES verification round (0 = verify all pending in one batch).",
    )
    parser.add_argument(
        "--tamper-index",
        type=int,
        default=-1,
        help="If >=0, tamper one queued packet (by index) to trigger invalid signature identification.",
    )
    ...
    def _single_run(_idx: int):
        ...
        payload = run_threaded_bmae_demo(
