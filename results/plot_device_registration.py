...
CSV_PATH = ROOT / "device_registration.csv"
...
def plot_for_security(rows: list[dict[str, str]], security_model: str):
    ...
    for scheme in SCHEME_ORDER:
        ...
        ax.errorbar(
            ...
            linewidth=2.0 if scheme == "BAT-CLAMA" else 1.4,
            markersize=5 if scheme == "BAT-CLAMA" else 4.2,
            capsize=2.2,
            elinewidth=0.85,
        )
    ...
    ax.set_title(f"Device Registration Time at {security_model}-bit Security")
    ax.grid(axis="y", linestyle="--", linewidth=0.45, alpha=0.35)
    ax.grid(axis="x", linestyle=":", linewidth=0.35, alpha=0.20)
    ...
    for scheme in SCHEME_ORDER:
        ...
        inset.errorbar(
            ...
            color=COLORS[scheme],
            marker=MARKERS[scheme],
            linewidth=1.3 if scheme == "BAT-CLAMA" else 1.0,
            markersize=3.4,
            capsize=1.6,
            elinewidth=0.65,
        )
    inset.set_xticks(COUNTS)
    inset.set_ylim(0, low_range_max * 1.12)
    inset.grid(axis="y", linestyle="--", linewidth=0.35, alpha=0.28)
    inset.set_title("Zoom", fontsize=7.2, pad=2)
