...
CSV_PATH = ROOT / "device_verification.csv"
...
# Paul Tol "Bright" — colorblind-safe, widely adopted in Nature / Science
COLORS = {
    "BAT-CLAMA":      "#EE6677",
    "PPT-CLAMA":      "#4477AA",
    "BMAE":           "#228833",
...
def plot_for_security(rows: list[dict[str, str]], security_model: str) -> None:
    ...
    ax.set_ylabel("Verification Time (ms)", labelpad=4)
    ax.set_xticks(COUNTS)
    ax.margins(x=0.06)
    ax.set_ylim(0, max_y * 1.10)

    #ax.set_title(
      #  f"Device Registration Time \u2014 {security_model}-bit Security",
      #  pad=7, fontweight="semibold", loc="left",
    #)
    ...
    # bbox_to_anchor is in axes-fraction coordinates; loc="center left" anchors
    # the legend box's left-center to that point.
    leg = ax.legend(
        ncol=1,
        frameon=True,
        framealpha=0.92,
    ...
    # the white facecolor occludes any background line passing underneath.
    ins = ax.inset_axes([0.08, 0.53, 0.36, 0.43])
    ...
    # ── Save ─────────────────────────────────────────────────────────────────
    out_path = OUT_DIR / f"device_verification_{security_model}bit.png"
